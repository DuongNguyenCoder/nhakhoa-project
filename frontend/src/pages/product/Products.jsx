...
const Products = () => {
  ...
  useEffect(() => {
    const fetchDirectory = async () => {
      await apiGetDirectory().then((res) => {
        if (res.data?.success) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Hiển thị Directory */}
      <div id="directory-list" className="bg-white shadow-xl rounded-2xl py-6 px-4 2xl:px-10">
        <div className="w-full flex gap-2 items-center">
          <CubeIcon className="size-7 text-yellow-400" />
          <h1 className="text-lg font-bold text-red-700 uppercase">Các Dòng sản phẩm</h1>
        </div>
        ...
        <div className="w-full grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-8 mt-4 px-3">
          {dataDirectory.length > 0 ? (
            dataDirectory.map((item) => (
              <div
                key={item._id}
                onClick={() => navigate(`/products/directory?directory=${item._id}&title=${encodeURIComponent(item.title)}`)}
            ...
            <p className="text-center text-gray-500 col-span-full">Không có dòng sản phẩm nào!</p>
          )}
        </div>
      </div>

      {/* Danh sách sản phẩm */}
      <div id="product-list" className="bg-white shadow-xl rounded-2xl py-6 px-4 2xl:px-10">
