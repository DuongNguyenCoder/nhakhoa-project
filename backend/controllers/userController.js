...
const createUserByAdmin = async (req, res) => {
  ...
  return res.json({
    ...
    mes: Boolean(updatedRecord) ? 'tạo thành công.' : 'xảy ra một lỗi vui lòng thử lại!',
  })
}
const updateUserByAdmin = async (req, res) => {
  ...
  return res.json({
    success: Boolean(updatedUser),
    mes: Boolean(updatedUser) ? 'sửa người dùng thành công.' : 'xảy ra một lỗi vui lòng thử lại!',
  })
}
...
`
  ...
  return res.json({
    ...
    mes: Boolean(response) ? 'vui lòng kiểm tra email của bạn.' : 'xảy ra một lỗi vui lòng thử lại',
  })
}

const checkForgotPassCode = async (req, res) => {
  const user = await User.findOne({ email: req.body.email }).select('forgotPassCode')
  const isMatch = req.body.code == user.forgotPassCode
  return res.json({
    success: Boolean(isMatch),
    mes: Boolean(isMatch) ? 'mã xác thực chính xác.' : 'mã xác thực sai. Vui lòng thử lại.',
  })
}
...
const resetPassword = async (req, res) => {
  ...
  return res.json({
    success: Boolean(response),
    mes: Boolean(response) ? 'Đổi mật khẩu thành công.' : 'xảy ra một lỗi vui lòng thử lại.',
  })
}
...
const clearCart = async (req, res) => {
  const response = await User.findByIdAndUpdate(req.user.id, { cart: [] }, { new: true })
  return res.json({
    success: Boolean(response),
    mes: Boolean(response) ? 'cập nhật giỏ hàng thành công.' : 'người dùng không tồn tại',
  })
}
