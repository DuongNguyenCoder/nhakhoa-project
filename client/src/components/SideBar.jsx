  ...
  return (
    ...
    <div className="w-full h-auto flex flex-col">
      <div className="flex h-10 items-center justify-center bg-red-700">
        <h2 className="text-center font-bold text-white">DANH SÁCH SẢN PHẨM</h2>
      </div>

      <div className="flex flex-col h-full divide-y">
        {dataDirectory.map((item, index) => (
          <div key={index} className="h-full border-t-orange-300 bg-orange-100">
            <div
              ...
              <div className="flex items-center w-full gap-x-3 md:gap-x-2 lg:gap-x-2.5">
                {item.directoryPic && (
                  <img
                    ...
                    className="lg:size-[30px] xl:size-[33px] rounded-full md:size-7 size-8"
                  />
                )}
                <Link
